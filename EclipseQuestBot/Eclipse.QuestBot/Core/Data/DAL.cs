    ...
    */
    public class DAL
    {
        ...
        public static void ExecuteSL3Query(string txtQuery, string FileName = null)
        {
           ...
           catch (Exception e) { EC.Log(e.ToString() + "\r\n " + txtQuery, LogLevel.Error); }
        }
        private static void SetSL3Connection(string curFile = null)
        {
            if (DBFile == null || DBFile == string.Empty) EC.FindDB();
            if (curFile != null)
            {
                if (!File.Exists(curFile)) curFile = Path.Combine(EC.StartupPath, curFile);
            ...
            {
                if (!File.Exists(DBFile)) DBFile = Path.Combine(EC.StartupPath, EC.DataPath);
                ...
                if (cols.Where(c => c.ToLower() == prop.Name.ToLower()).Count() > 0) // we want to skip ID since it usually cant be inserted... and make sure the property exists in the column names
                {
                    ...
                    else
                    {
                        ...
                        catch (Exception err)
                        {
                            ...
                            EC.Log(string.Format("Cannot serialize property {0} - {1}",prop, err.Message), LogLevel.Error);
