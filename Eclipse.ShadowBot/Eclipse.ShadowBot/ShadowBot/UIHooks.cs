...
namespace Eclipse.ShadowBot
{
    ...
    public static class UIHooks
    {
        ...
        public static void AttachUIEvents()
        {
            ...
            if (EclipseShadowBot.PickUpQuests) Lua.Events.AttachEvent("QUEST_GREETING", OnQuestGreeting);
            if (EclipseShadowBot.PickUpQuests) Lua.Events.AttachEvent("QUEST_DETAIL", OnQuestDetail);
            if (EclipseShadowBot.PickUpQuests) Lua.Events.AttachEvent("GOSSIP_SHOW", OnGossipShow);
            if (EclipseShadowBot.PickUpQuests) Lua.Events.AttachEvent("QUEST_COMPLETE", OnQuestComplete);
            if (EclipseShadowBot.PickUpQuests) Lua.Events.AttachEvent("QUEST_PROGRESS", OnQuestProgress);
            ...
            Lua.Events.AttachEvent("MERCHANT_SHOW", OnMerchantWindowShow);
        }
        public static void DetatchUIEvents()
        {
            EC.Log("Deattaching quest events.");
            if (EclipseShadowBot.PickUpQuests) Lua.Events.DetachEvent("QUEST_GREETING", OnQuestGreeting);
            if (EclipseShadowBot.PickUpQuests) Lua.Events.DetachEvent("QUEST_DETAIL", OnQuestDetail);
            if (EclipseShadowBot.PickUpQuests) Lua.Events.DetachEvent("GOSSIP_SHOW", OnGossipShow);
            if (EclipseShadowBot.PickUpQuests) Lua.Events.DetachEvent("QUEST_COMPLETE", OnQuestComplete);
            if (EclipseShadowBot.PickUpQuests) Lua.Events.DetachEvent("QUEST_PROGRESS", OnQuestProgress);

            Lua.Events.DetachEvent("PARTY_INVITE_REQUEST", OnPartyInvite);
            ...
            Lua.Events.DetachEvent("MERCHANT_SHOW", OnMerchantWindowShow);
        }
        #endregion
        
        #region Lua Event Handlers
        public static void OnMerchantWindowShow(object sender, LuaEventArgs args)
        {

            EC.SellingAtVendor = true;
            List<WoWItem> objList = ObjectManager.GetObjectsOfType<WoWItem>().Where(o => o.Quality == WoWItemQuality.Poor).OrderBy(o => o.Distance).ToList();
            EC.Log(string.Format("Going to sell {0} items.", objList.Count));
            foreach (WoWItem o in objList)
            {
                if (TreeRoot.IsRunning)
                {
                    EC.Log(string.Format("Selling {0}", o.Name));
                    MerchantFrame.Instance.SellItem(o);
                   // CommonCoroutines.SleepForRandomUiInteractionTime();
                }
            }
            EC.SellingAtVendor = false;
        }
        ...
        private static void OnPartyInvite(object sender, LuaEventArgs args)
        {
            ...
            EC.Log(string.Format("Accepting group invite from {0}", invitedBy));
        ...
        {
            EC.Log("Ready check has been initiated by " + args.Args[0]);
        ...
        {
            EC.Log("Someone wants to rez you...");
            Lua.DoString("RunMacroText(\"/click StaticPopup1Button1\")");
        }
