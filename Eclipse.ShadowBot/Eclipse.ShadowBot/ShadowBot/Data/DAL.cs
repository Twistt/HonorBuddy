...
namespace ArachnidCreations
{
    ...
    */
    public class DAL
    {
        ...
        public static void CreateSL3Connection(string curFile = null)
        {
            if (curFile != null)
            {
                if (!File.Exists(curFile))
                {
                    ...
                    dbConn = new SQLiteConnection(@"Data Source=" + curFile + ";Version=3;New=False;Compress=True;"); 
                }
            }
        }
        private static void SetSL3Connection(string curFile = null)
        {
            ...
            {
                if (!File.Exists(DBFile)) DBFile = Path.Combine(Application.StartupPath, EC.DataPath);
                ...
                if (cols.Where(c => c.ToLower() == prop.Name.ToLower()).Count() > 0) // we want to skip ID since it usually cant be inserted... and make sure the property exists in the column names
                {
                    ...
                    else
                    {
                        ...
                        catch (Exception err)
                        {
                            ...
                            EC.Log(string.Format("Cannot serialize property {0} - {1}",prop, err.Message));
