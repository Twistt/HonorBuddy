        ...
        internal static void FindDB()
        {
            ...
            if (!File.Exists(DataPath))
            {
                ...
                } else
                {
                    ...
                    DialogResult dialogResult = MessageBox.Show("You must have the EclipseWoWDB file for the Botbase to work, would you like HB to download it automaticly? (it can take up to 2 minutes)", "Required file for EclipseBotBase", MessageBoxButtons.YesNo, MessageBoxIcon.Question);

                    if (dialogResult == DialogResult.Yes)
                    {

                            WebClient webClient = new WebClient();
                            webClient.DownloadFile("http://www.arachnidcreations.com/HonorBuddy/EclipseWoWDB.zip", "EclipseWoWDB.edb");
                        }
                 
                    else if (dialogResult == DialogResult.No)
                    {
                        Logging.Write("Please download them here: http://www.arachnidcreations.com/HonorBuddy/EclipseWoWDB.zip be sure to change the .zip extension to .edb");
                    }
