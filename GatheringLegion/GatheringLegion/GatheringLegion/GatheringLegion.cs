...
namespace Eclipse.Bots.GatheringLegion
{
    public class EclipseGatheringLegion : BotBase
    {
        ...
        {
            get { return "Eclipse - GatheringLegion Ver 0.1"; }
        }
                ...
                return _root ?? (_root =
                    new PrioritySelector(
                        ...
                        new Decorator(ret => !Core.PassiveMode && !StyxWoW.Me.IsGhost,
                            new PrioritySelector(
                                new Decorator(d=> EC.GatherChests && CheckForChests(),  CreateTreasureChestBehavior()),
        ...
        private static bool GetNearestMiningNode()
        {
            ...
            foreach (var obj in objects)
            {
                ...
                {
                    if (o.IsHerb && !SpellManager.HasSpell("Herbalism")) continue;
                    if (o.IsMineral && !SpellManager.HasSpell("Mining")) continue;
                    if (EC.AvoidElites)
                    {
        ...
        public static Location GetNextLocation()
        {
            ...
            foreach (var _loc in _locList)
            {
                ...
                if (distance < 40)
                {
                    EC.log(string.Format("within {0} of a hotspot- and thers still nothing here, so we are gonna add this to recently visited.", distance));
                    Core.RecentlyVisitedLocations.Add(_loc);
                    ...
                    new PrioritySelector(
                        new Decorator(ret => Core.GatherMode,
                            new PrioritySelector(
                                ...
                                new Decorator(ret => SpellManager.HasSpell("Skinning"), 
                                ...
                                new Decorator(ret =>SpellManager.HasSpell("Skinning"), 
                                    new Decorator(d=> Me.CurrentTarget != null || TargetClosestSkinnableMob(),CreateSkinningBehavior())
                                )
                            )
                        )
                    )
