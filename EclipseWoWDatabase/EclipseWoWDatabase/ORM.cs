    ...
    public class ORM
    {
        ...
        public static string Insert(object userClass, string tablename, string fieldprefix = "", bool returnId = false, DataTable dt = null)
        {
            ...
            foreach (PropertyInfo prop in matchedProps)
            {
                ...
                if (prop.Name.ToLower() != "id" && cols.Where(c => c.ToLower() == prop.Name.ToLower()).Count() > 0) // we want to skip ID since it usually cant be inserted... and make sure the property exists in the column names
                {
        ...
        public static string Update(object userClass, string tablename, string fieldprefix = "", DataTable dt = null)
        {
            ...
            if (property == null) property = userClass.GetType().GetProperty("Entry");
            sql.Append(string.Format(" where entry='{0}';", property.GetValue(userClass)));
            return sql.ToString();
        }
        /// <summary>
        /// 
